...
ArgumentHelper::ArgumentHelper(Interpreter* inter_arg, const lax::Arguments* aargs)
    : inter(inter_arg)
    , args(aargs)
    , next_argument(0)
...
namespace detail
{
    ...
    template<> std::string get_from_obj_or_error<std::string>(std::shared_ptr<Object> obj) { return lax::get_string_from_obj_or_error(obj); }
    template<> bool get_from_obj_or_error<bool>(std::shared_ptr<Object> obj) { return lax::get_bool_from_obj_or_error(obj); }
    template<> Tf get_from_obj_or_error<Tf>(std::shared_ptr<Object> obj) { return lax::get_float_from_obj_or_error(obj); }
    template<> Ti get_from_obj_or_error<Ti>(std::shared_ptr<Object> obj) { return lax::get_int_from_obj_or_error(obj); }

    template<> std::shared_ptr<Object> make_object<std::string>(std::string str) { return lax::make_string(str); }
    template<> std::shared_ptr<Object> make_object<bool>(bool b) { return lax::make_bool(b); }
    template<> std::shared_ptr<Object> make_object<Tf>(Tf n) { return lax::make_number_float(n); }
    template<> std::shared_ptr<Object> make_object<Ti>(Ti n) { return lax::make_number_int(n); }
...
fmt::formatter<lax::SingleArg>::format(const lax::SingleArg& a, fmt::format_context& ctx) const
{
    // todo(Gustav): replace with a better intention?
    const std::string s = a.type ? a.type->to_flat_string(nullptr, nullptr, lax::ToStringOptions::for_print()) : "<?>";
