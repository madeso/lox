...
void
AddErrorErrors::on_errors(const lax::Offset& o, const std::vector<std::string>& messages)
{
    errors->emplace_back(ReportedError{ReportedError::Type::error, o.start, o.end, messages});
}

void
AddErrorErrors::on_notes(const lax::Offset& o, const std::vector<std::string>& messages)
{
